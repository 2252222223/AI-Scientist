...
class CEO_GPT:
    """Agent class for interacting with CEO-GPT."""
    ...
    def __init__(
            ...
            tools: List[BaseTool],
            Expert_experience: str = None,
            Expert_experience_switch: bool=False,
        ...
        self.Expert_experience_switch = Expert_experience_switch
    ...
    def from_llm_and_tools(
            ...
            Expert_experience_switch: bool =False,
            Expert_experience: str = None,
            feedback_tool: Optional = None,
            output_parser: Optional[BaseAutoGPTOutputParser] = None,
            chat_history_memory: Optional[BaseChatMessageHistory] = None,
    ) -> CEO_GPT:
        prompt = CEO_GPT_Prompt(
            ai_name=ai_name,
            ai_role=ai_role,
            tools=tools,
        ...
        human_feedback_tool = feedback_tool if COB_in_the_loop else None
    ...
    def run(self, goals: List[str]) -> str:
        def execute_tool():
            ...
            if action.name in tools:
                ...
                try:
                    print("******************")
                    print(action.args)
                    print(type(action.args.get(list(action.args.keys())[0])))
                    next_action = action.args
                    if type(next_action.get(list(action.args.keys())[0])) == str:
                        pass
                    else:
                        abs = str(next_action.get(list(next_action.keys())[0]))
                        next_action = {list(next_action.keys())[0]: abs}
                    print("------------------------")
                    print(next_action)
                    observation = tool.run(next_action)
                    # print(observation)
        ...
        cob_command =("")
        if self.Expert_experience_switch :
            expert_experience_ma = expert_experience_match(goals, self.Expert_experience)
        ...
        while True:
            ...
            if self.feedback_tool is not None:
                CEO_decision = assistant_reply
                report_to_COB = f"Honorable COB, the following is the CEO's decision report:" \
                                f"{CEO_decision}" \
                                f"" \
                ...
                if "YES" in feedback.upper():
                    ...
                    cob_instructions = f"The following are the Chairman's instructions, which you must obey unconditionally. Instructions:{feedback}"
